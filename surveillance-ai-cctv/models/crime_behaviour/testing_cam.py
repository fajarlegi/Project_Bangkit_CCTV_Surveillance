...
pose_model = YOLO("models\crime_behaviour\weight\yolov8n-pose.pt")
...
def process_frame(frame):
    ...
    for det in detected_sunglasses:
        ...
        if should_log_detection(det, "Sunglasses"):
            ...
            url = save_to_minio(image_bytes, "cobacctv", f"sunglasses_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
            if url:
                crime_data.append({
    ...
    for det in detected_helmets:
        ...
        if should_log_detection(det, "Helmet"):
            ...
            image_bytes = cv2.imencode('.jpg', frame)[1].tobytes()
            url = save_to_minio(image_bytes, "cobacctv", f"helmet_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
            if url:
                crime_data.append({
                    'msg': 'Helmet Detected',
                    'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
    ...
    for pose in pose_results[0].keypoints.data:
        ...
        if pose.shape[0] >= 17:
            ...
            if left_leg_angle < 90 and right_leg_angle < 90:
                ...
                image_bytes = cv2.imencode('.jpg', frame)[1].tobytes()
                url = save_to_minio(image_bytes, "cobacctv", f"squat_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
                if url:
                    crime_data.append({
                        'msg': 'Squat Detected',
                        'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
...
#     return render_template('crime_cam.html', crime_data=crime_data)
...
# @app.route('/crime-cam')
# def index():
#     # Render the template with crime data
